...
def _print_status_message(icon: str, msg: str, style: Union[str, Style]):
    ...
    console.print(
        f"[{str(style)}]{icon}[/{str(style)}]  [{str(style)}]{msg}[/{str(style)}]"
    )


def _print_stdout_panel(
    stdout: Optional[str], title: str, border_style: Union[str, Style]
):
...
def _print_traceback_panel(traceback_str: Optional[str]):
    ...
    if traceback_str:
        ...
        console.print(
            _create_panel(syntax, title="[red]Traceback[/red]", border_style=FAIL_STYLE)
        )
...
def print_test_case_result(
    ...
    status_text = Text(
        f"{status_icon} {'PASSED' if passed else 'FAILED'}",
...
def print_test_error(
    ...
    show_traceback = (
        detailed
        and traceback_str
        and traceback_str.strip()
        and traceback_str.strip() != error_msg.strip()
    )
    if show_traceback:
        syntax = Syntax(
            traceback_str, "python", theme="monokai", line_numbers=True, word_wrap=True
        )
    ...
    console.print(
        _create_panel(
            content, title="[red]Error[/red]", border_style=FAIL_STYLE, padding=(1, 2)
        )
    )
...
def print_summary(total_passed: int, total_run: int, selected: int, total: int):
    ...
    progress_bar.add_task("Tests Passed", total=total_run, completed=total_passed)
...
def print_test_summary_table(test_results: List[Dict[str, Any]]):
    ...
    for result in test_results:
        ...
        table.add_row(str(result["case_num"]), status, time_str, mem_str)
    console.print(table)
...
def print_profile_summary_table(profiled_results: List[Dict[str, Any]]):
    ...
    for result in profiled_results:
        table.add_row(
            str(result["case_num"]),
            (
                format_time(result["avg_time"] / 1000)
                if result["avg_time"] is not None
                else "N/A"
            ),
            (
                format_time(result["min_time"] / 1000)
                if result["min_time"] is not None
                else "N/A"
            ),
            (
                format_time(result["max_time"] / 1000)
                if result["max_time"] is not None
                else "N/A"
            ),
            (
                format_memory(int(result["avg_mem_bytes"]))
                if result["avg_mem_bytes"] is not None
                else "N/A"
            ),
            (
                format_memory(int(result["max_mem_bytes"]))
                if result["max_mem_bytes"] is not None
                else "N/A"
            ),
...
def print_snapshot_list(
    snapshots: List[Dict[str, Any]], language: str, challenge_path: str
):
...
def print_snapshot_comparison(
    snapshot1_info: Dict[str, Any],
    snapshot2_info: Dict[str, Any],
    diff_lines: List[str],
):
...
def print_performance_comparison(performance_data: Dict[int, Dict[str, Any]]):
    ...
    for case_num, data in performance_data.items():
        ...
        time_diff_color = (
            "green" if time_diff_pct < 0 else "red" if time_diff_pct > 0 else "white"
        )
        mem_diff_color = (
            "green" if mem_diff_pct < 0 else "red" if mem_diff_pct > 0 else "white"
        )
