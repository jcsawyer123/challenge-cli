...
class HistoryVisualizer:
    ...
    def __init__(self, challenge_dir: str, language: str):
        ...
        self.performance_file = os.path.join(
            self.history_dir, "performance", f"{language}.json"
        )
        self.test_results_file = os.path.join(
            self.history_dir, "test_results", f"{language}.json"
        )
    ...
    def _generate_performance_chart_data(
        self, case_filter: Optional[List[int]] = None
    ) -> Dict:
        """
    ...
    def _generate_test_results_chart_data(self) -> Dict:
        ...
        return {"timestamps": timestamps, "passed": passed, "failed": failed}
