class ComplexityAnalyzer:
    ...
    def analyze_file(self, file_path):
        ...
        for node in solution_class.body:
            if isinstance(node, self.ast.FunctionDef) and node.name != "__init__":
                ...
                time_complexity, space_complexity, data_structures, is_recursive = (
    ...
    def _analyze_function(self, func_node):
        ...
        time_complexity = self._determine_time_complexity(
            loop_info, is_recursive, data_structures
        )
    ...
    def _generate_explanation(
        ...
        if time_complexity == "O(1)":
            ...
            explanation.append(
                "  • The solution uses a fixed number of operations regardless of input size."
            )
            ...
            explanation.append(
                "  • The solution likely uses nested loops or quadratic algorithms."
            )

        elif time_complexity == "O(n log n)":
            explanation.append("Time Complexity: O(n log n)")
            explanation.append(
                "  • The solution likely uses sorting or a divide-and-conquer approach."
            )
            ...
            explanation.append(
                f"  • The solution uses {power} nested loops or similar polynomial complexity."
            )

        elif time_complexity == "O(2^n)":
            explanation.append("Time Complexity: O(2^n) - Exponential time")
            explanation.append(
                "  • The solution likely uses recursive calls that branch at each step."
            )
        ...
        if space_complexity == "O(1)":
            ...
            explanation.append(
                "  • The solution uses a fixed amount of extra space regardless of input size."
            )

        elif space_complexity == "O(n)":
            explanation.append("\nSpace Complexity: O(n) - Linear space")
            explanation.append(
                "  • The solution's memory usage grows linearly with the input size."
            )
            ...
            explanation.append(
                f"  • The solution's memory usage grows with the {power}th power of the input size."
            )
        ...
        if time_complexity in ["O(n²)", "O(n^3)", "O(2^n)"]:
            ...
            if not data_structures["dict_or_set"]:
                explanation.append(
                    "  • Consider using hash maps (dictionaries) for O(1) lookups."
                )
            if (
                time_complexity == "O(2^n)" and is_recursive
            ):  # is_recursive is now directly available
                explanation.append(
                    "  • Consider memoization to avoid redundant recursive calculations."
                )
